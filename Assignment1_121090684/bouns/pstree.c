 ...
 * @param pid The process ID for which to read threads.
 ...
 * @param sibling_map Array of flags indicating whether the previous siblings at each depth were last children.
 */
void printProcessTree_c(pid_t pid, int depth, int is_last_child, int *sibling_map)
{
    ...
    // Traversal threads
    readThreadsInProc(pid, threads, &thread_count);
    int count = 0;
    char pre_name;
    char curr_name;
    for (int i = 1; i < thread_count; i++)
    {
        print_spaces(depth + 1, i == thread_count - 1, sibling_map);
        // printf("proc: %s ", proc_name);
        // printf("thread: %s_%d", threads[i].name, i);
    ...
    {
        printProcessTree_c(children[i].pid, depth + 1, i == child_count - 1, sibling_map);
    }
}

void printProcessTree_p(pid_t pid, int depth, int is_last_child, int *sibling_map)
{
    char proc_name[MAX_NAME_LEN] = "Unknown";
    int len = 0; // Length of the last process name printed
    bool have_child = false;
    ...
    for (int i = 0; i < child_count; i++)
    {
        printProcessTree_p(children[i].pid, depth + 1, i == child_count - 1, sibling_map);
    }
}

void printProcessTree_T(pid_t pid, int depth, int is_last_child, int *sibling_map)
{
    char proc_name[MAX_NAME_LEN] = "Unknown";
    int len = 0; // Length of the last process name printed
    bool have_child = false;
    ...
    for (int i = 0; i < child_count; i++)
    {
        printProcessTree_T(children[i].pid, depth + 1, i == child_count - 1, sibling_map);
    }
}
...
int main(int argc, char *argv[])
{
    ...
    else if (argc == 2)
    {
        if (strcmp(argv[1], "-c") == 0)
        {
            ...
            printProcessTree_c(1, 0, 1, sibling_map);
        }
        else if (strcmp(argv[1], "-p") == 0)
        {
            int sibling_map[MAX_NAME_LEN] = {0};
            printProcessTree_p(1, 0, 1, sibling_map);
        }
        else if (strcmp(argv[1], "-T") == 0)
        {
            int sibling_map[MAX_NAME_LEN] = {0};
            printProcessTree_T(1, 0, 1, sibling_map);
        }
    }
    else
        printf("Invalid input\n");
