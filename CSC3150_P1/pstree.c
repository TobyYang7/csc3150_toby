...
void processArray(char **arr, int size)
{
    ...
    for (int i = 1; i < size; ++i)
    {
        ...
        else
        {
            if (count > 1)
            {
                char *processedItem = (char *)malloc(20); // Adjust the size as needed
                sprintf(processedItem, "%d*[%s]", count, currentChar);
                result[resultSize++] = processedItem;
            }
            else
            {
                result[resultSize++] = strdup(currentChar);
            }
    ...
    {
        printf("%s ", result[i]);
        free(result[i]); // Free allocated memory for each item
    }
...
void readThreadsInProc(pid_t pid, ChildProcess *threads, int *thread_count)
{
...
/**
 * Compare function for sorting ChildProcess array by name.
 * @param a Pointer to first ChildProcess struct.
 * @param b Pointer to second ChildProcess struct.
...
void printProcessTreeDFS(pid_t pid, int depth, int is_last_child, int *sibling_map)
{
    ...
    ChildProcess threads[MAX_NAME_LEN];
    int thread_count = 0;
