    ...
    11d8:	00e6ea63          	bltu	a3,a4,11ec <free+0x50>
{
  ...
  if(bp + bp->s.size == p->s.ptr){
    ...
    11f2:	02059813          	slli	a6,a1,0x20
    11f6:	01c85713          	srli	a4,a6,0x1c
  ...
  if(p + p->s.size == bp){
    ...
    1206:	02061593          	slli	a1,a2,0x20
    120a:	01c5d713          	srli	a4,a1,0x1c
...
malloc(uint nbytes)
{
  ...
  if(p == (char*)-1)
    ...
    1292:	b7e1                	j	125a <malloc+0x36>
      if(p->s.size == nunits)
    1294:	02e48c63          	beq	s1,a4,12cc <malloc+0xa8>
        p->s.size -= nunits;
    1298:	4137073b          	subw	a4,a4,s3
    129c:	c798                	sw	a4,8(a5)
        p += p->s.size;
    129e:	02071693          	slli	a3,a4,0x20
    12a2:	01c6d713          	srli	a4,a3,0x1c
    12a6:	97ba                	add	a5,a5,a4
        p->s.size = nunits;
    12a8:	0137a423          	sw	s3,8(a5)
      freep = prevp;
    12ac:	00001717          	auipc	a4,0x1
    12b0:	d6a73223          	sd	a0,-668(a4) # 2010 <freep>
      return (void*)(p + 1);
    ...
    12d0:	bff1                	j	12ac <malloc+0x88>
  hp->s.size = nu;
    12d2:	01652423          	sw	s6,8(a0)
  free((void*)(hp + 1));
    12d6:	0541                	addi	a0,a0,16
    12d8:	00000097          	auipc	ra,0x0
    12dc:	ec4080e7          	jalr	-316(ra) # 119c <free>
  return freep;
    12e0:	00093503          	ld	a0,0(s2)
      if((p = morecore(nunits)) == 0)
    12e4:	d971                	beqz	a0,12b8 <malloc+0x94>
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    12e6:	611c                	ld	a5,0(a0)
    if(p->s.size >= nunits){
    12e8:	4798                	lw	a4,8(a5)
    12ea:	fa9775e3          	bgeu	a4,s1,1294 <malloc+0x70>
    if(p == freep)
    12ee:	00093703          	ld	a4,0(s2)
    12f2:	853e                	mv	a0,a5
    12f4:	fef719e3          	bne	a4,a5,12e6 <malloc+0xc2>
  p = sbrk(nu * sizeof(Header));
    12f8:	8552                	mv	a0,s4
    12fa:	00000097          	auipc	ra,0x0
    12fe:	b6c080e7          	jalr	-1172(ra) # e66 <sbrk>
  if(p == (char*)-1)
    1302:	fd5518e3          	bne	a0,s5,12d2 <malloc+0xae>
        return 0;
    1306:	4501                	li	a0,0
    1308:	bf45                	j	12b8 <malloc+0x94>
