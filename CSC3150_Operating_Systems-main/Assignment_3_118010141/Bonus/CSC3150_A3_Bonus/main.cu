...
// 128 KB in global memory -- disk，IO buffer
#define STORAGE_SIZE (1 << 17)

//// count the pagefault times
// managed: can be accessed from both host and device
__device__ __managed__ int pagefault_num = 0; 
__device__ __managed__ int pagefault_count = 0;
...
__global__ void mykernel(int input_size) {
  ...
  VirtualMemory vm;
  int page_count = 0;
  vm_init(&vm, data, storage, pt, &pagefault_num, page_count, PAGE_SIZE,
